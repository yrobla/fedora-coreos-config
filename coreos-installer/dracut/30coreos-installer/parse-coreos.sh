...
local INJECT_ON_KERNEL=$(getarg coreos.inst.ignition_on_kernel_params=)
if [ ! -z "$INJECT_ON_KERNEL" ]
then
    echo "inject ignition on kernel params $INJECT_ON_KERNEL" >> /tmp/debug
    echo $INJECT_ON_KERNEL >> /tmp/inject_on_kernel
fi
